...
def Ajouter():
    ...
    try:
        sql = "INSERT INTO note (matricule, nom, prenom, sexe, classe, matiere, note) VALUES (%s, %s, %s, %s, %s, %s, %s) "
...
def Modifier():
    ...
    try:
        sql = "update note set nom=%s, prenom=%s, sexe=%s, classe=%s, matiere=%s, note=%s where code=%s"
